...
export default function SubHeader({ props }: { props: SubHeaderInterface }) {
  ...
  if (props.title_path_1 === "") {
    ...
    title_part_1 = (
      <Link
        to={props.title_path_1}
        className="font-sans self-center text-medium font-bold text-white hover:text-main_yellow"
      >
        {props.title_1}
      </Link>
    );
  ...
  if (props.title_2 !== "") {
    ...
    title_complete = <div className="flex flex-row">{title_part_1}</div>;
  }
  ...
  if (props.text_settings !== "" || props.text_add !== "") {
    ...
    balise = (
      <div className="flex justify-between bg-background px-base py-sm">
        {title_complete}
        {buttons}
      </div>
    );
